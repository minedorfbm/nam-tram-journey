...
export const getHubData = createServerFn({ method: "GET" }).handler(async (): Promise<HubData> => {
  ...
  const [levels, destinations, photos, settings] = await Promise.all([
    ...
    supabase
      .from("destinations")
      .select(
        "id, name, level_id, cluster, type, short_description, image_key, discover_url, menu_url, vegetarian_menu_url, vegan_menu_url, lunch_menu_url, dinner_menu_url, booking_url, instagram_url, booking_message, display_order, active",
      )
      .eq("active", true)
      .order("display_order"),
    supabase
